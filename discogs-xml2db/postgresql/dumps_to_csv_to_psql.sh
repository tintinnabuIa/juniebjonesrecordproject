...
python3 "${SCRIPT_DIR}/discogs-xml2db/run.py" --bz2 --output=csv_dir "${SCRIPT_DIR}/discogs-xml2db/discogs_dumps" --export="artist" --export="label" --export="master" --export "release"
...
sed -e "s/discogs/juniebjonesrecordproject/" -e "s/postgres/${whoami}/" -e "s/pgpass/${whoami}/" "${SCRIPT_DIR}/discogs-xml2db/postgresql/postgresql.conf"
